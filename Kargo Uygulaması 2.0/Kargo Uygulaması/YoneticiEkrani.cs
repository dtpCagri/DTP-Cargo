    ...
    public partial class YoneticiEkrani : Form
    {
        ...
        private void btn_SilmeyiTamamla_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string SilmeKomutu = "select * from Personel where SicilNo = @SicilNo";
        ...
        {
            TChatasi.Clear();
            btn_PersonelGuncelleSorgula.Enabled = false;
            btn_PersonelGuncellemeIslemi.Enabled = false;

            if (txt_PersonelGuncelleTC.Text != "")
                PersonelGuncelleTCkontrolü(txt_PersonelGuncelleTC.Text);
        }

        private void txt_PersonelGuncelleTC_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        // Güncelle butonu tıklanırsa gerekli işlemler
        private void btn_PersonelGuncellemeIslemi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string YeniDeger = txt_PersonelGuncelleYeniDeger.Text;
                string Sorgu = "";

                switch (DegisecekAlan)
                {
                    case "Ad": Sorgu = "update Personel set Ad = @YeniDeger where TCnumara = @TC"; break;
                    case "Soyad": Sorgu = "update Personel set Soyad = @YeniDeger where TCnumara = @TC"; break;
                    case "Eposta": Sorgu = "update Personel set Eposta = @YeniDeger where TCnumara = @TC"; break;
                    case "Telefon": Sorgu = "update Personel set TelefonNo = @YeniDeger where TCnumara = @TC"; break;
                    case "SicilNo": Sorgu = "update Personel set SicilNo = @YeniDeger where TCnumara = @TC"; break;
                }

                baglanti.Open();
                SqlCommand Guncelle = new SqlCommand(Sorgu, baglanti);
                // Guncelle.Parameters.AddWithValue("@YeniDeger", YeniDeger);
                Guncelle.Parameters.AddWithValue("@TC", txt_PersonelGuncelleTC.Text);
                Guncelle.Parameters.AddWithValue("@YeniDeger", YeniDeger);
                Guncelle.ExecuteNonQuery();
                SqlDataReader Dr = Guncelle.ExecuteReader();
                MessageBox.Show("Bilgi Güncellenmiştir", "BAŞARILI", MessageBoxButtons.OK, MessageBoxIcon.Information);
