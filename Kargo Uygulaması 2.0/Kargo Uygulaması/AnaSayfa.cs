    ...
    public partial class AnaSayfa : Form
    {
        ...
        private void btn_PaketiOlustur_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // İl ilce mahalle keylerine ulaşma
                string AliciSehirKeySorgu = "select * from sehir where sehir_title = @SehirAd";
                baglanti.Open();
                int AliciSehirKey = 0;
                SqlCommand AliciSehirID = new SqlCommand(AliciSehirKeySorgu, baglanti);
                AliciSehirID.Parameters.AddWithValue("@SehirAd", YeniAlici.Il);
                SqlDataReader AliciSehirDR = AliciSehirID.ExecuteReader();
                if (AliciSehirDR.Read())
                {
                    AliciSehirKey = Convert.ToInt32(AliciSehirDR["sehir_key"]);
                }
                baglanti.Close();

                string AliciIlceKeySorgu = "select * from ilce where ilce_title = @IlceAd";
                baglanti.Open();
                int AliciIlceKey = 0;
                SqlCommand AliciIlceID = new SqlCommand(AliciIlceKeySorgu, baglanti);
                AliciIlceID.Parameters.AddWithValue("@IlceAd", YeniAlici.Ilce);
                SqlDataReader AliciIlceDR = AliciIlceID.ExecuteReader();
                if (AliciIlceDR.Read())
                {
                ...
                string AliciMahalleKeySorgu = "select * from mahalle where mahalle_title = @MahalleAd";
                baglanti.Open();
                int AliciMahalleKey = 0;
                SqlCommand AliciMahalleID = new SqlCommand(AliciMahalleKeySorgu, baglanti);
                AliciMahalleID.Parameters.AddWithValue("@MahalleAd", YeniAlici.Mahalle);
                SqlDataReader AliciMahalleDR = AliciMahalleID.ExecuteReader();
                if (AliciMahalleDR.Read())
                {
                ...
                string AliciEkle = "insert into Musteri(Musteri_Ad, Musteri_Soyad, Musteri_TelefonNo, Musteri_Eposta, Musteri_Statü, Musteri_TamAdres, Musteri_SehirKey, Musteri_IlceKey, Musteri_MahalleKey) values (@Ad, @Soyad, @Telefon, @Eposta, 'Alıcı', @TamAdres, @SehirKey, @IlceKey, @MahalleKey)";
                ...
                {
                    MessageBox.Show("Gönderici ve Alıcı Bilgisi Sisteme Eklenmiştir", "BAŞARILI", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
